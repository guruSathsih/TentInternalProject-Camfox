<?php		
	...
	class API extends REST {		
		...
		public function new_post()
		{
			...
			$post_id = newPost($post_content, $user_id, 'text');
			...
			$this->setNotificationDeatils($friends, "Added new post...");
			...
			if($post_id != 0)
			{
				$result.= '<div  class="post_wrap"><a id="'.$post_id.'" onClick="show_post_detail(this)">';
					...
					$result.= '<div class="post_details"> ';
						...
						if(strlen($post_content) > 25)
							$result.= '<div class="post_text"> '.substr($post_content,0,20).'...<a id="'.$post_id.'" style="font-size: 11px; color: #197700 !important;  margin: -17px 6px -2px 17px;" name="'.$post_id.'_readmore" onclick="show_post_detail(this) ">Read More </a></div>';
						else
		...
		public function show_post_content()
		{
			...
			if($post_content != null)
			{
				foreach($post_content as $post){
					$result.= '<div  class="post_wrap"><a id="'.$post[0].'" >';
						$result.= '<div class="post_wrap_author_profile_picture">';
							if($post[6] != null)
						...
						$result.= '<div class="post_details"> ';
							...
							{
								if(strlen($post[2]) > 25)
									$result.= '<div class="post_text"> '.substr($post[2],0,20).'...<a id="'.$post[0].'" style="font-size: 11px; color: #197700 !important;  margin: -17px 6px -2px 5px;" name="'.$post[0].'_readmore" onclick="show_post_detail(this) ">Read More </a></div>';
								else
									$result.= '<div class="post_text"> '.$post[2].' </div>';
							}
							else if($post[7] == 'audio')
							{
								$result.='<audio controls>';
									//$result.= '<source src="horse.ogg" type="audio/ogg">';
									$result.= '<source src="http://creatustent.com/camfox/process/uploads/audios/'.$post[2].'" type="audio/mpeg">';
								$result.= '</audio> ';
							}
							else if($post[7] == 'video')
							{
								$result.= '<video width="250" height="240" controls>';
								  $result.= '<source src="http://creatustent.com/camfox/process/uploads/videos/'.$post[2].'" type="video/mp4">';
								  //$result.= '<source src="movie.ogg" type="video/ogg">';
								$result.= '</video> ';
							}
							else if($post[7] == 'image')
							{
								$result.= '<div class="post_image" ><img src="http://creatustent.com/camfox/process/uploads/images/'.$post[2].'" /> </div>';
							} 
							else
							{
		...
		function uploadImage(){ 
				...
				$fileName = $user_id."_".$timestamp.".jpg";  
				
					$post_content = $fileName;
					$post_type = $_POST['post_type']; 
					$now_time = $timestamp;	
					echo "inside upload image";
		...
		function store_registrationId()
		{
			...
			$reg_id = $_POST['reg']; 
			storePushNotificationId($reg_id, $user_id);
			$this->http_response($reg_id,200); 
		}
			...
			try{		
				...
				if(mysql_num_rows($notify) > 0){
					while($row = mysql_fetch_array($notify)){
						$to = $row['push_notify_id']; //registration ID of the device
						$title = $userdetails[0];
						$message = $msg;
		...
		function sendPushNotification($to,$title,$message)
		{
			...
			// replace API
			//define( 'API_ACCESS_KEY', 'AIzaSyDBjDEUFS-7rdaRWelQ252AgUg9w5Gt0e8');
			if (!defined('API_ACCESS_KEY')) define('API_ACCESS_KEY', 'AIzaSyDBjDEUFS-7rdaRWelQ252AgUg9w5Gt0e8');
		...
		{			
			...
			if($users_list != null )
			{
				...
				$shuffled_list = $this->shuffleThis($users_list);

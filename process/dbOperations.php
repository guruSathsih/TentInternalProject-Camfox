...
function newPost($content, $user_id)
{
	$conn=getDBConnection();
	$current_date = date("Y-m-d H:i:s");
	$insertQuery="insert into posts(content, user_id, 	status, date_time, 	total_comments, total_likes) values('$content', '$user_id', 'new', '$current_date', 0, 0)";
	if (!mysql_query($insertQuery,$conn))
	{
	  return 0;
  	}
